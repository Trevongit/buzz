...
| **Pocket Listen** | Experiment `pocketListen` | **on** | Speak, pause/resume, wait for a named Reader summary, Follow along. Off hides Listen controls. **Listen** works without the Reader in the channel. **Listen (summary)** needs that agent as a channel member so they can post. |
...
- [Trevongit/agy-cli-ide-bridge](https://github.com/Trevongit/agy-cli-ide-bridge) — Antigravity CLI ↔ Antigravity IDE glue (sibling; not extras, not UATP)
