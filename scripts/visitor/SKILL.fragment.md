...
bash "$ROOT/post.sh" --room <id> --from "$VISITOR_ROLE" --to <peer> --task "…"
# BLOCKED to Prime only when stuck:
bash "$ROOT/post.sh" --room <id> --from "$VISITOR_ROLE" --to grok --task "…" --status BLOCKED --need-prime true
```
...
- Prime is disturbed only on `status: BLOCKED` + `need_prime: true`.
