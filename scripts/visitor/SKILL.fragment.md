...
Then say **use buzz**. One install covers Grok, Codex, and agy on this machine.

Works from **any workspace** on this computer. Do not use the folder name as the seat
(`agy-uni-adapt` is not a Buzz seat). Set an existing seat:
...
- Codex workflow: TUI listen-only. L2 is Buzz evidence. Tight pair with agy uses `collab.sh --dry-run` (TTY) then one L2 post. Turbo is house spawn, not this seat. Failed posts retry at most 3 times then stop (no unsee). `auto-reply.log` is redacted.
