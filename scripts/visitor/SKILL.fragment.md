...
- Antigravity workflow: CLI TUI and Antigravity IDE are listen-only. L2 is Buzz evidence (`auto-reply.sh --seat agy-buzz`, `agy --print`). `use buzz` in the IDE is this skill when `~/.agy/skills` is attached — same seat, no mint, no second L2. CLI↔IDE glue is a sibling repo (https://github.com/Trevongit/agy-cli-ide-bridge), not extras and not UATP. Do not Custom-harness the IDE.
